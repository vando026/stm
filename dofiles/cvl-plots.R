...
plotCVL <- function(dat, main, cex2=1) {
  ...
  with(dat, plotCI(Age, mean, ui=ub, li=lb, xlim=c(-0.1, 6.5), 
    xlab="", ylab="", xaxt="n", yaxt="n", axes=FALSE, 
    lwd=2, cex=scex, pch=21, pt.bg=scols, col=scols, main=main))
...
over50 <- transform(over50, Age = ifelse(Data=="CVL", Age + 0.15, Age - 0.15))
