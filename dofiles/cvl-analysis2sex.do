...
** Set 3
foreach mod of global PCVLm {
  eststo `mod': stcox `mod' $prev $vars if Female==0, noshow
}
global opts4 order($PCVLm)
...
***********************************************************************************************************
** Set 1
global prev "i.HIV_pcat_Male"
eststo mm: stcox G_PVL_Male if Male==0, noshow
esttab mm using "$output/Model_PVL_Unad_Mal.`opts5'", $opts11 $opts12 replace
...
** Set 2
global PCVLm G_PVL_Male P_PDV_Male P_CTI_Male 
global opts4 order($PCVLm)
foreach mod of global PCVLm {
  eststo `mod': stcox `mod' $vars if Female==0, noshow
}
